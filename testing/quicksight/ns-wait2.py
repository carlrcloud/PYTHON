...
def lambda_handler(event, context):
    ...
    try:
        ...
        if request_type == 'Create':
            def wait_for_account_subscription(max_retries=20, delay=30):
                for _ in range(max_retries):
                    try:
                        response = quicksight.describe_account_subscription(
                            AwsAccountId=aws_account_id
                        )
                        logger.info('Describe account subscription response: %s', response)
                        if 'SubscriptionStatus' in response:
                            status = response['SubscriptionStatus']
                            logger.info('Account subscription status: %s', status)
                            if status == 'ACTIVE':
                                return True
                        else:
                            logger.warning('SubscriptionStatus not found in response: %s', response)
                    except quicksight.exceptions.ResourceNotFoundException as e:
                        logger.warning('Account subscription not found: %s', e)
                ...
                for _ in range(max_retries):
                    try:
                        ...
                        if 'CreationStatus' in response:
                            ...
                            logger.warning('CreationStatus not found in response: %s', response)
                    except quicksight.exceptions.ResourceNotFoundException as e:
            ...
            # Wait for Account Subscription to be active
            if not wait_for_account_subscription():
                raise Exception('Account subscription is not active after retries')
